...
- 生成时间：2026-08-28T15:29:48+08:00
- 数据源：`/home/shadow/.dsh/slm-shadow/session-slm-shadow.jsonl`（总行数 403，其中预测失败 54 条）
- 覆盖天数：3

**GATE: MET**（门槛：N≥100 或 天数≥3；当前 N=349）

| 指标 | 值 |
|------|----|
| N（有效影子条数 predict_ok=true） | 349 |
| 建议弱档率 | 258（73.9%） |
| 实际弱档率 | 234（67.0%） |
| 一致率 agree=true | 189（54.2%） |
| 本该换弱 switch_to_weak（省成本机会） | 64（18.3%） |
| 本该换强 switch_to_strong（质量风险·优先抽检） | 67（19.2%） |
| 目标不健康率（would_bind 且 target unhealthy） | 0（0.0%） |
| 弃权率 abstained=true | 42（12.0%） |
| p95 predict_ms | 239ms |

## 抽检建议

- 存在 67 条「本该换强」——按计划优先抽检这些轮次（质量风险）。筛选命令：`grep switch_to_strong <log>` 后对照 utterance_preview。
