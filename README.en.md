...
6. **Health-aware dead-anchor detection**: the plugin tracks per-provider failure classes in-session. Once the parent's route is judged unhealthy (`quota`/`auth` are terminal; transient classes expire after a 60s TTL), later `model: "auto"` calls **drop the anchor** and pick a healthy provider instead — children are no longer pinned to a broken route. The `subagent_models` catalog tool annotates each provider with `health: healthy/unhealthy` + `failingClass` + `retryAfterSec`.
