...
| [dsh&#8209;subagent&#8209;router](./dsh-subagent-router) | **Model-routed Delegation v0.2.0**:<br>· `subagent_model`: per-call provider/model/max_tokens overrides<br>· built-in `model: "auto"` routing (anchored to parent · task-tier upgrade · failure escalate/reroute · health-aware dead-anchor detection · auditable)<br>· four-layer routing priorities (provider order · tier policy · tier picks · budget ceiling)<br>· **settings-page config UI** (Settings → Plugins, live without restart)<br>· `subagent_models` catalog tool (with provider health) | ✅ published (GitHub + npm) |
