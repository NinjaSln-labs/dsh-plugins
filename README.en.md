...
| [dsh&#8209;subagent&#8209;cursor](./dsh-subagent-cursor) | **Cursor-as-subagent v0.1.0**:<br>· registers a `ctx.subagents` provider (default `cursor`) via `@cursor/sdk`<br>· one-shot local · summary-first result presentation · Profile Bundle<br>· scaffold stage (`start()` not implemented yet) | 🚧 local only (not git-added) |
