...
V1.0 初始提案 → V1.1 完整规格（契约/数据流/备选/AC/测试矩阵）
 ...
 → V1.11 终审（两项 minor 关闭确认；红队残留扫描零发现；279/279 行双语一致）
```
...
| **V1.11** | **终审（PASS, 0 remaining）** | 两项 minor 关闭确认；红队残留扫描零发现；双语 279/279 行一致 |
...
|---|---|---|
| L1 recall@1（hard 难查询） | ≥30% 均值（无单次 <20%） | 动态原型 43%/64%；宿主 bundle L1-live 64%（方差 10 轮待跑） |
