...
> 状态：计划（待执行）· 日期：2026-08-21 · 目标：让 `dsh-subagent-router` 的配置出现在 **设置 → 插件配置** 页面（`settings.plugin.item` 卡片），并支持在 UI 编辑、保存生效。
...
| 表单渲染 | 自绘 React 表单（20 字段），不用 `dsh-client-schema-form`（不在 profile 依赖链） | 可控、无额外依赖风险 |
...
- [ ] A1. `package.json`：`peerDependencies` 加 `@deepseek-ai/dsh-settings`（dsh 主程序实际版本 **0.1.0-rc.8**；peer 声明宽松 `^0.1.0-rc.8`，运行时解析到 dsh 共享 node_modules）
- [ ] A2. `src/index.ts`：`apply` 里用 `installSettingsSection(ctx, settingsNamespace('subagent-router'), Config, config, { setSource, onChange })` 注册；引入 `currentConfig()` 响应式引用
- [ ] A3. `resolveConfig` 保持纯函数（schema 归一化值或 entry 都走它）；`apply` 的 `resolved` 改为每次从 `currentConfig()` 读取
- [ ] A4. `registerModelPickerTools(ctx, config)` 改为接受 getter `() => ResolvedModelPickerConfig`（工具闭包内每次读最新）
- [ ] A5. 单测：host 注册（mock settings 服务或真 dsh-settings）、响应式更新（setSource 后 getter 变化）、向后兼容（无 settings 时用 entry）

### Phase B — Client 侧卡片
- [ ] B1. 新建 `src/client.tsx`：默认导出 client 插件
...
- [ ] B2. 表单字段映射：20 个 schema 字段 → 控件（`autoProviderOrder` 数组编辑、`autoTierPolicy` 三档 enum、`maxDepth` 数字/enum 等）
- [ ] B3. client 端错误处理：scope 不可用（`status: 'unavailable'`）时卡片渲染降级提示

### Phase C — 打包链与元数据
- [ ] C1. 新建 `scripts/build-client.mjs`（照 compass：esbuild → `lib/client.js`，`__ModuleLoader__` banner/footer，EXTERNAL 列表 + `@deepseek-ai/dsh-client-runtime/client`）
- [ ] C2. `package.json`：`exports["./client"]`、`dsh.client.inject`（`@deepseek-ai/dsh-client-runtime`）、peer deps 加 client 运行时、`build` 脚本改为 `tsc && node scripts/build-client.mjs`
- [ ] C3. `tsconfig.build.json`：确认 client.tsx 编译通过（tsc 仅类型检查 client，打包靠 esbuild）
- [ ] C4. 依赖安装：`@deepseek-ai/dsh-settings`（host peer，rc.8）、`esbuild`（dev）、`@types/react`（dev）、react（peer）

### Phase D — 验证（自测）
- [ ] D1. 单测：`src/config.ts` schema 与 `resolveConfig` 双源一致（已有）；新增 host settings 注册测试
- [ ] D2. tsc typecheck 通过（host + client 类型）
- [ ] D3. esbuild client 打包成功，`lib/client.js` 生成
- [ ] D4. 全量测试绿（现有 64 + 新增）
- [ ] D5. 部署 profile（lib + package.json 同步）→ 重启 dsh web

### Phase E — 验收（真实环境）
- [ ] E1. 设置 → 插件配置 出现 `subagent-router` 卡片
- [ ] E2. 卡片显示 20 个字段表单，当前值正确（来自 patch.yml base + schema 默认）
- [ ] E3. 编辑字段（如 `autoTierPolicy.trivial` 改 `cheapest`）→ 保存 → 生效（`model: "auto"` 实测 policy 变化）
- [ ] E4. 重置字段（unset）→ 回退到 patch.yml 值
- [ ] E5. `subagent_model` 工具在配置变化后行为正确（响应式生效，无需重启）
- [ ] E6. 无回归：现有验证场景（锚定/升级/换路）仍工作

## 4. 验收标准（DoD）

1. **设置页可见**：设置 → 插件配置 中显示 `subagent-router` 卡片（非空）。
2. **表单完整**：20 个配置字段全部可编辑；类型正确（布尔/数字/字符串/数组/enum）。
3. **读值正确**：卡片初始值 = patch.yml base（当前 `autoProviderOrder: [deepseek-official, amd, opencode-go]` + `autoTierPolicy` 三档）⊕ schema 默认。
...
| 20 字段表单开发量大 | 按类型分组渲染（text/bool/number/array/enum 五类控件），复用单类组件 |
...
- 文档：`README.md`/`README.en.md`（配置说明补「设置页 UI」）、`docs/ROADMAP.md`（1b 完成）、`HANDOFF.md`（delta）
