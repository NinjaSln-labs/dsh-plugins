...
function row(id, title, createdAt, status, health, workspace) {
  return {
    id,
    title,
    status,
 ...
 * (PAGE_SIZE = 5): the 明/暗 × 四档 matrix baseline. Also spans all three
 * activity states (运行中/已加载/冷却) so the 状态 column is covered.
 */
export const FIVE_TIER_ROWS = [
  row('s-red', '重构合并会话', 100, 'running',
    healthOf('red', {
  ...
  row('s-yellow', '知识库接入调研', 300, 'loaded',
    healthOf('yellow', {
