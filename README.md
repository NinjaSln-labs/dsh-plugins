...
| [dsh-subagent-model-picker](./dsh-subagent-model-picker) | model-chosen subagent delegation: `subagent_model` tool with per-call provider/model/max_tokens overrides + built-in `model: "auto"` auto-selection (tier → catalog pick, failure escalation, auditable reason) + `subagent_models` catalog tool | published (GitHub + npm) |
