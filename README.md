...
| [dsh&#8209;subagent&#8209;cursor](./dsh-subagent-cursor) | **Cursor 子代理 v0.1.0**：<br>· 经 `@cursor/sdk` 注册 `ctx.subagents` provider（默认 `cursor`）<br>· one-shot local · summary 优先结果展示 · Profile Bundle<br>· 脚手架阶段（SDK `start()` 尚未实现） | 🚧 本地未入库（未 git add） |
