...
|---|---|---|
| [dsh-session-health](./dsh-session-health) | session health v0.5.4: reactive badge (projection-driven), `/health` command, `session_health` tool, official peak/off-peak dual-currency pricing (CNY/USD, jsdelivr + GitHub-raw fallback), theme-adaptive severity palette, cache-aware window-scaled economy tier, money/token cost toggle, automated handoff checklist | published (GitHub + npm) |
