...
| [dsh&#8209;subagent&#8209;router](./dsh-subagent-router) | **子代理模型路由 v0.2.0**：<br>· `subagent_model`：每次调用可指定 provider/model/max_tokens<br>· 内置 `model: "auto"` 路由策略（锚定父模型 · 任务分档升级 · 失败升档/换路 · 健康感知死锚检测 · 全程可审计）<br>· 路由优先级四层配置（provider 序 · 档位策略 · 档位候选 · 预算封顶）<br>· **设置页配置 UI**（设置 → 插件配置，保存即实时生效，无需重启）<br>· `subagent_models` 目录工具（含 provider health） | ✅ 已发布（GitHub + npm） |
